...
namespace MirrorDriver
{
    public class DesktopMirror : IDisposable
    {
        ...
        public class DesktopChangeEventArgs : EventArgs
        {
            ...
            public DesktopChangeEventArgs(Rectangle region, OperationType operationType)
            {
                this.Region = region;
        ...
        {
            var getChangesBuffer = (GetChangesBuffer)Marshal.PtrToStructure(_getChangesBuffer, typeof(GetChangesBuffer));
            var data = new byte[Screen.PrimaryScreen.Bounds.Width * Screen.PrimaryScreen.Bounds.Height];
